...
export class Hud {
  ...
  _buildSettings() {
    ...
    for (const id of PLANETS_ORDER) {
      ...
      b.innerHTML = `<i class="orb" style="background:radial-gradient(circle at 33% 30%, ${c1}, ${c2})"></i><span style="font-weight:600">${planet.name}</span><span style="font-size:10px;color:#34d399">${planet.gravityLabel || '1.0g'}</span><span style="font-size:9.5px;color:#f59e0b">${planet.bulkOfWork ? planet.bulkOfWork.slice(0, 24) : ''}</span>`
  ...
  setSelection(agent, thread) {
    ...
    const activePlanet = PLANETS[this.settings.get('planet')] || PLANETS.moon
    bits.push(`<div class="planet-domain-tag" style="width:100%;font-size:10px;color:#f59e0b;margin-top:3px">🪐 <b>${escapeHtml(activePlanet.name)} Domain (${escapeHtml(activePlanet.gravityLabel)}):</b> ${escapeHtml(activePlanet.bulkOfWork || '')}</div>`)
